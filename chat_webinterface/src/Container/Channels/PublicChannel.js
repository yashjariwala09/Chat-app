...
class PublicChannelList extends Component {
    constructor(props) {
        super(props)
        this.state = {
            hideList: false
    ...
    render() {
        ...
        if (userList && userList.length) {
            userList.forEach((value, key) => {
                row.push(
                    <ChannelWrapper key={"public_" + key + value.name}>
        ...
        return (
            <div>
                <div className="Channel-header">
                    <div className="toggle-header" onClick={this.handdleHideList} data-toggle="collapse" data-target="#collapse_public_channel">
                        <DropArrowWrapper hide={!hideList}>
                            <DropArrow height={30} width={30} fill={"#6B6C71"} />
                        </DropArrowWrapper>
                        <ChannelHeader headerTtle={"public Channel"} fontSize={16} color={"#6B6C71"} />
                    </div>
                    <span style={{ marginLeft: "auto", overflow: "hidden" }}><PlusIcon height={24} width={24} fill={"white"} /></span>

                </div>
                <div className="collapse message-list-collapse" id="collapse_public_channel">
                    {row}
                </div>
            </div>
        )
